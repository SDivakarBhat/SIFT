...
def detect(image, threshold = 5):
	# SIFT Detector 
	#--------------
     
	#original = ndimage.imread(imagename, flatten=True)
	original = cv2.cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
	# SIFT Parameters
	s = 3
	k = 2 ** (1.0 / s)
	...
	for i in range(1, 4):
		for j in range(80, doubled.shape[0] - 80):
			for k in range(80, doubled.shape[1] - 80):
				...
				maxv = (diffpyrlvl1[j, k, i] > 0)
				minv = (diffpyrlvl1[j, k, i] < 0)
					...
					for dj in range(-1, 2):
						for dk in range(-1, 2):
							if di == 0 and dj == 0 and dk == 0:
								continue
						maxv = maxv and (diffpyrlvl1[j, k, i] > diffpyrlvl1[j + dj, k + dk, i + di])
						minv = minv and (diffpyrlvl1[j, k, i] < diffpyrlvl1[j + dj, k + dk, i + di])
						if not maxv and not minv:
							break
				...
				det = (dxx * dyy - (dxy ** 2))
				r = 10.0
				if ((( trace** 2) * r) < det * (((r + 1) ** 2))) and (np.absolute(x_hat[0]) < 0.5) and (np.absolute(x_hat[1]) < 0.5) and (np.absolute(x_hat[2]) < 0.5) and (np.absolute(D_x_hat) > 0.03):
					extrpyrlvl1[j, k, i - 1] = 1
					temp = cv2.KeyPoint(x_hat[0], x_hat[1],7)
					keypoint1.append(temp)
	...
	for i in range(1, 4):
		for j in range(40, normal.shape[0] - 40):
			...
			maxv = (diffpyrlvl2[j, k, i] > 0)
			minv = (diffpyrlvl2[j, k, i] < 0)
			...
			for di in range(-1, 2):
				for dj in range(-1, 2):
					for dk in range(-1, 2):
						if di == 0 and dj == 0 and dk == 0:
							continue
					maxv = maxv and (diffpyrlvl2[j, k, i] > diffpyrlvl2[j + dj, k + dk, i + di])
					minv = minv and (diffpyrlvl2[j, k, i] < diffpyrlvl2[j + dj, k + dk, i + di])
					if not maxv and not minv:
						break
	...
	for i in range(1, 4):
		for j in range(20, halved.shape[0] - 20):
			...
			maxv = (diffpyrlvl3[j, k, i] > 0)
			minv = (diffpyrlvl3[j, k, i] < 0)
			...
			for di in range(-1, 2):
				for dj in range(-1, 2):
					for dk in range(-1, 2):
						if di == 0 and dj == 0 and dk == 0:
							continue
					maxv = maxv and (diffpyrlvl3[j, k, i] > diffpyrlvl3[j + dj, k + dk, i + di])
					minv = minv and (diffpyrlvl3[j, k, i] < diffpyrlvl3[j + dj, k + dk, i + di])
					if not maxv and not minv:
						break
	...
	for i in range(1, 4):
		for j in range(10, quartered.shape[0] - 10):
			for k in range(10, quartered.shape[1] - 10):
				...
				maxv = (diffpyrlvl4[j, k, i] > 0)
				minv = (diffpyrlvl4[j, k, i] < 0)
				...
				for di in range(-1, 2):
					for dj in range(-1, 2):
						for dk in range(-1, 2):
							if di == 0 and dj == 0 and dk == 0:
								continue
						maxv = maxv and (diffpyrlvl4[j, k, i] > diffpyrlvl4[j + dj, k + dk, i + di])
						minv = minv and (diffpyrlvl4[j, k, i] < diffpyrlvl4[j + dj, k + dk, i + di])
						if not maxv and not minv:
							break
					if not maxv and not minv:
						break
			...
			for k in range(80, doubled.shape[1] - 80):
				if extrpyrlvl1[j, k, i] == 1:
					...
					while newmaxval > 0.8 * maxval:
						...
						#keypoints.append(np.array([[int(j * 0.5), int(k * 0.5), kvectotal[i], newmaxidx]]))
			...
			for k in range(40, normal.shape[1] - 40):
				if extrpyrlvl2[j, k, i] == 1:
					...
					while newmaxval > 0.8 * maxval:
						...
						#keypoints.append(np.array([[int(j ), int(k ), kvectotal[i+3], newmaxidx]]))
						orient_hist[newmaxidx] = 0
						newmaxval = np.amax(orient_hist)
			...
			for k in range(20, halved.shape[1] - 20):
				if extrpyrlvl3[j, k, i] == 1:
					...
					while newmaxval > 0.8 * maxval:
						...
						#keypoints.append(np.array([[int(j * 2), int(k * 2), kvectotal[i+6], newmaxidx]]))
						orient_hist[newmaxidx] = 0
						newmaxval = np.amax(orient_hist)
			...
			for k in range(10, quartered.shape[1] - 10):
				if extrpyrlvl4[j, k, i] == 1:
					...
					while newmaxval > 0.8 * maxval:
						...
						#keypoints.append(np.array([[int(j * 4), int(k * 4), kvectotal[i+9], newmaxidx]]))
						orient_hist[newmaxidx] = 0
						newmaxval = np.amax(orient_hist)
	...
	for i in range(0, keypoints.shape[0]): 
		for x in range(-8, 8):
			for y in range(-8, 8):
				...
				weight = magpyr[int(x0 + xrot), int(y0 + yrot), int(scale_idx)] * gaussian_window.pdf([x0 + xrot, y0 + yrot])
	...
	#keypoints = cv2.KeyPoint(keypoints)
	#print(type(keypoints))
	"""keypoints1 =  (keypoints.tolist())
	dest = []
	for k in keypoints1:
		key = cv2.KeyPoint(k.x, k.y)
		dest.append(key)"""
	#print(type(keypoints))
	#img = np.zeros_like(image)
	#img = cv2.drawKeypoints(image, keypoint1, img)
